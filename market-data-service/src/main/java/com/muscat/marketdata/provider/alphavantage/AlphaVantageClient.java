...
public class AlphaVantageClient {
  ...
  public JsonNode get(String function, Map<String, String> params) {
    ...
    try {
      ...
      if (response == null || response.trim().isEmpty()) {
        ...
        marketDataLogger.logApiCall("ALPHAVANTAGE", function, symbol, false, responseTime,
            "Empty response");
      ...
      if (jsonResponse.has("Note") && jsonResponse.get("Note").asText()
          .contains("call frequency")) {
      ...
      log.error("AlphaVantage API 호출 중 예상치 못한 오류: function={}, error={}", function, e.getMessage(),
          e);
      marketDataLogger.logApiCall("ALPHAVANTAGE", function, symbol, false, responseTime,
          e.getMessage());
  ...
  private String handleHttpError(HttpClientErrorException e, String function, String symbol,
      long responseTime) {
    ...
    switch (status) {
      ...
      default:
        errorMessage = String.format("AlphaVantage API 오류 [%d]: %s", status.value(),
            e.getMessage());
