...
public class ComparisonCalculator {
    ...
    public static ComparisonCalculationResult calculate(List<ComparisonItem> items, String analysisType) {
        ...
        // 요약 생성
        String summary = String.format("%s 결과: 최고 %s (%.2f%%), 최저 %s (%.2f%%), 평균 수익률 %.2f%%",
            analysisType,
            bestPerformer.getName(), bestPerformer.getTotalReturnPercent(),
            worstPerformer.getName(), worstPerformer.getTotalReturnPercent(),
            averageReturn);
