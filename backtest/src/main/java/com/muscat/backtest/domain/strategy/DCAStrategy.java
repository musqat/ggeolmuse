...
public class DCAStrategy implements InvestmentStrategy {
  ...
  public StrategyResponse execute(StrategyRequest request) {
    ...
    validateDCASpecificRequest(request);
    ...
    try {
      ...
      int investmentDay = request.getInvestmentDay() != null ? request.getInvestmentDay() : 1;
  ...
  private List<StrategyTransaction> executeRegularInvestments(
      StrategyRequest request, LocalDate actualEndDate, int investmentDay) {
