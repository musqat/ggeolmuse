...
public class TradeUtils {
    ...
    public BigDecimal calculateFee(AccountBalanceDto accountBalance, BigDecimal tradeAmount) {
        ...
        log.debug("기본 수수료율 사용: 거래금액={}, 기본수수료율={}%", 
                tradeAmount, tradeProperties.getFee().getDefaultRate().multiply(TradeConstants.PERCENTAGE_MULTIPLIER));
    ...
    public AccountBalanceDto getAccountBalance(String accountId) {
        try {
            var response = userServiceClient.getAccountBalance(Long.valueOf(accountId));
            if (response == null) {
