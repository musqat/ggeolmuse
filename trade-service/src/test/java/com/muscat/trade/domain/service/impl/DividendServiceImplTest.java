...
class DividendServiceImplTest {
    ...
    void setUp() {
        ...
        testDividend = Dividend.builder()
                ...
                .netAmount(TEST_QUANTITY.multiply(DIVIDEND_PER_SHARE).multiply(BigDecimal.ONE.subtract(TAX_RATE)))
    ...
    class GetDividendsWithCacheTests {
        ...
        void getDividendsWithCache_NewDividend_Success() {
            ...
            // 배당이 아직 캐시되지 않음
            given(dividendRepository.existsByTradeIdAndDividendDate(TEST_TRADE_ID, LocalDate.parse(testDividendDto.exDate())))
                    .willReturn(false);

            given(dividendRepository.findByUserIdAndSymbolOrderByDividendDateDesc(TEST_USER_ID, TEST_SYMBOL))
            ...
            Map<String, Object> dividend = result.get(0);
    ...
    class RefreshDividendCacheTests {
        ...
        void refreshDividendCache_Success() {
            ...
            given(dividendRepository.findByUserIdAndSymbolOrderByDividendDateDesc(eq(TEST_USER_ID), anyString()))
                    .willReturn(new ArrayList<>());
