...
public class MarketDataServiceImpl implements MarketDataService {
    ...
    try {
      ...
      if (response != null && response.available()) {
        BigDecimal price = switch (priceType) {
          case OPEN -> response.openPrice();
          case HIGH -> response.highPrice();
          case LOW -> response.lowPrice();
          case CLOSE -> response.closePrice();
  ...
  public BigDecimal validateManualPrice(String symbol, LocalDate tradeDate,
    ...
    try {
      ...
      if (response != null && response.available()) {
        BigDecimal lowPrice = response.lowPrice();
        BigDecimal highPrice = response.highPrice();
