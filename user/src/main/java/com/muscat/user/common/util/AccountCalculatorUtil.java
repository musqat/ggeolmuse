...
public class AccountCalculatorUtil {
  ...
  public ExchangeCalculationResult calculateExchangeWithCommission(
    ...
    return switch (fromCurrency + "_TO_" + toCurrency) {
      case "KRW_TO_USD" -> ExchangeCalculationResult.ofKrwToUsd(amount, exchangeRate, account.getCommissionRate());
      case "USD_TO_KRW" -> ExchangeCalculationResult.ofUsdToKrw(amount, exchangeRate, account.getCommissionRate());
