    ...
    public List<CandleDto> fetchDailyAdjusted(String symbol, LocalDate from, LocalDate to) {
        try {
            ...
            if (!response.has("Time Series (Daily)")) {
                ...
                throw new DataNotFoundException(symbol, "CANDLE");
            }
            ...
            marketDataLogger.logDataCollection("ALPHAVANTAGE", symbol, "CANDLE", candles.size(), true, null);
            ...
            marketDataLogger.logDataCollection("ALPHAVANTAGE", symbol, "CANDLE", 0, false, e.getMessage());
            throw new MarketDataException(errorMsg + ": " + symbol, e);
        }
    }
