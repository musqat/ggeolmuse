...
| 2026-08-26 | 서비스 | [로그인 응답이 가입 여부를 드러냈다](#로그인-응답이-가입-여부를-드러냈다) | 인증 여부를 비밀번호보다 먼저 검사 | 없음 — 응답 시간 차이는 남음 |
| 2026-08-27 | 서비스 | [운영이 dev 프로파일로 판정됐다](#운영이-dev-프로파일로-판정됐다) | System.getProperty 로 프로파일 판정 | 세 서비스 수정 |
| 2026-08-25 | 통신 | [서킷브레이커가 한 번도 열리지 않았다](#서킷브레이커가-한-번도-열리지-않았다) | recordExceptions 에 RestTemplate 예외 | OPEN · 미등록 알림 |
...
| 2025-10-15 | 게이트웨이 | [타임아웃인데 405 가 나왔다](#타임아웃인데-405-가-나왔다) | fallback 컨트롤러 부재 · GetMapping | 메서드별 테스트 |
| 2026-08-25 | 게이트웨이 | [처리량 상한이 앱이 아니라 게이트웨이였다](#처리량-상한이-앱이-아니라-게이트웨이였다) | replenishRate 200 · 429 를 실패로 셈 | k6 가 429 를 따로 센다 |
| 2026-08-26 | 게이트웨이 | [rate limit 이 IP 별이 아니라 전체로 걸렸다](#rate-limit-이-ip-별이-아니라-전체로-걸렸다) | getRemoteAddress 가 프록시 IP | 앱 한계 측정 완료 |
...
| 2025-11-26 | 인프라 | [16GB 노드에서도 메모리가 부족했다](#16gb-노드에서도-메모리가-부족했다) | -Xmx 미설정 | 메모리 알림 |
...
| 2026-06-03 | 인프라 | [새 서비스만 503 이 났다](#새-서비스만-503-이-났다) | NetworkPolicy 허용 포트에 8000 없음 | CI 가 포트 대조 |
| 2026-08-28 | 인프라 | [서비스 다섯이 한꺼번에 죽었다](#서비스-다섯이-한꺼번에-죽었다) | config-server 만 기본 비밀번호로 기동 | 401 알림 |
| 2026-08-28 | 인프라 | [Flyway 체크섬이 두 개 어긋나 있었다](#flyway-체크섬이-두-개-어긋나-있었다) | 적용된 마이그레이션 수정 · 검증 꺼짐 | 검증 켬 |
| 2026-08 | 인프라 | [Tempo 가 재시작 때 OOM 으로 죽었다](#tempo-가-재시작-때-oom-으로-죽었다) | 쌓인 트레이스 볼륨 | 익명 볼륨 |
| 2026-08-26 | 로컬 | [모든 AWS 명령이 빈 응답을 냈다](#모든-aws-명령이-빈-응답을-냈다) | credentials 파일에 제어문자 | 없음 |

재발 방지 칸이 "없음" 인 것이 26건 중 6건이다. 고친 것과 다시 안 나게 한 것은 다르다.
...
`재발 방지`
알림 둘을 뒀다. `CircuitBreakerOpen` 은 네 서비스 어디서든 서킷이 1분 넘게 열리면 알림이 가고,
`CircuitBreakerMissing` 은 기대하는 (서비스, 이름) 메트릭이 15분 넘게 없으면 알림이 간다. 뒤엣것이
trade-service 의 이름 어긋남을 잡는 쪽이다. 전에는 게이트웨이 것만 있었다.
...
`재발 방지`
`FallbackControllerTest` 가 GET · POST · PUT · DELETE · PATCH 로 `/fallback` 을 쳐서 전부
503 과 안내 본문이 오는지 본다. 게이트웨이의 첫 테스트다.
...
2026-09-06 에 같은 VPC 의 별도 EC2 에서 다시 쟀다. 측정 PC 가 포화하는 문제가 없어진다.

```
price 직접             342 rps   파드 CPU 한도 700m 에 붙음
price 게이트웨이        143 rps   앱의 42%
symbols 포함 혼합 부하   33.6 rps
```

건당 CPU 로 보면 price 는 2ms, symbols 는 63ms 다. symbols 한 번이 price 30번어치다.
...
`재발 방지`
2026-09-06 에 앱 한계를 쟀다. 같은 VPC 의 c5.xlarge 에서 k6 로 쐈다.

```
price 직접(8083)      342 rps   p95 197ms
price 게이트웨이(8070) 143 rps   p95 312ms   429 0건
```

rate limit 은 200/s 인데 게이트웨이가 143 rps 에서 먼저 찼다. 지금 통과량을 정하는 것은
rate limit 값이 아니라 게이트웨이 자체다. 버킷이 쪼개져 늘어난 통과량도 그 아래에 있다.
...
`재발 방지`
CI 가 렌더한 매니페스트에서 NetworkPolicy 의 게이트웨이 규칙 포트와 `tier: business-service`
워크로드의 `containerPort` 를 대조한다(`.github/scripts/check-netpol-ports.sh`). 빠진 포트가
있으면 `scan-manifests` 잡이 실패한다.
...
`재발 방지`
config-server 가 5분 넘게 401 을 내면 `ConfigServerUnauthorized` 알림이 간다(critical).
비밀번호가 양쪽에서 갈리면 클라이언트가 기동마다 401 을 받으니 그걸 본다. 환경변수 이름이
쿠버네티스 자동 주입 접두어와 겹치는 것(`CONFIG_SERVER_PORT` 등)은 그대로다.

<br>

## Flyway 체크섬이 두 개 어긋나 있었다
...
`재발 방지`
2026-09-06 에 설정 리포에서 `validate-on-migrate: false` 를 지웠다. 네 서비스가 검증을
켠 채로 뜬다. 적용된 파일이 바뀌면 그 서비스가 기동에서 멈춘다.

켜는 과정에서 세 서비스가 더 걸렸다. backtest 만의 일이 아니었다.

```
user-service          V1              주석
trade-service         V1, V3          주석 · 미커밋 이미지로 적용
market-data-service   V1, V2          주석 · 인덱스 정의
```

같은 방식으로 `flyway_schema_history` 의 checksum 행만 갱신했다. 그 전에 trade V3 는
컬럼과 PK 를, market V2 는 인덱스 정의를 직접 확인했다.

뿌리는 적용된 마이그레이션 파일을 고친 것과 커밋 안 된 파일로 이미지를 구운 것 둘이다.
앞엣것은 규칙으로, 뒤엣것은 CI 에서만 굽는 것으로 막힌다.

<br>

## Tempo 가 재시작 때 OOM 으로 죽었다
